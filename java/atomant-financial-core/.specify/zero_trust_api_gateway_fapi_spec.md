...
The architectural diagram depicts a **Zero-Trust** environment where the API Gateway acts as the single point of entry and enforcement. 

Key architectural components include:
1.  **External Client (Open Banking)**: Communicates via FAPI-compliant protocols (HTTPS + OAuth 2.0 + PKCE).
2.  **API Gateway**: 
    - Terminates external TLS.
    - Validates OAuth codes and PKCE verifiers.
    - Issues/Proxies JWT tokens.
    - Enforces **mTLS** for all downstream requests.
3.  **Internal Microservices**: Only accept connections via mTLS. They validate the JWT propagated by the gateway using **SmallRye JWT**.

### Security Handshake Flow (Mermaid)
```mermaid
sequenceDiagram
    participant Client as External Open Banking Client
    ...
    Note over Client, Gateway: FAPI / OAuth 2.0 + PKCE
    Client->>Gateway: 1. Request Auth Code (code_challenge)
    Gateway->>Auth: Forward Auth Request
    Auth-->>Client: 2. Auth Code Issued

    Client->>Gateway: 3. Exchange Code (code_verifier)
    Gateway->>Auth: Validate Verifier & Issue Token
    Auth-->>Gateway: Access Token (JWT)
    Gateway-->>Client: Access Token (JWT)

    Note over Gateway, Service: Internal mTLS + JWT
    Client->>Gateway: 4. API Request (Bearer JWT)
    Gateway->>Gateway: Validate external TLS
    Gateway->>Service: 5. Route Request (mTLS + JWT)
    Service->>Service: 6. SmallRye JWT Validation
    Service-->>Gateway: 7. Resource Data
    Gateway-->>Client: 8. Secure API Response
```
